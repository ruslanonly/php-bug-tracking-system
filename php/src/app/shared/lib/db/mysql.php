<?php
/*
    DatabaseManager for Mysqli v1.6
    == Usage ============================
    1. Backward compatible version:
       $db = new DatabaseManager(db_host, db_user, db_pass, db_schm);
    2. Support of charaset, disable debug message
       $db = new DatabaseManager(db_host, db_user, db_pass, db_schm, FALSE, 'utf8');
    ...
    == Program History ==================
    original DatabaseManager for MySQL by Raptor Kwok
    original DatabaseManager for MySQLi by Hoyu

    Feel free to use, but kindly leave this statement here.

    Technical Support : findme@raptor.hk ( please specify "DatabaseManager for MySQLi" in title )
*/
...
class DatabaseManager
{
    ...
    public function DatabaseManager($host, $user, $pass, $dbname = '', $_debugMode = true, $charSet = 'utf8', $autoCommit = true, $port = 3306, $persistent = false)
    {
        $this->debugMode = $_debugMode;
    ...
    public function connect($host, $user, $pass, $dbname, $persistent, $charSet, $autoCommit, $port)
    {
        ...
        if ($this->mysqli->connect_error !== null) {
            if ($this->debugMode) {
                $this->halt('Connect Error (' . $this->mysqli->connect_errno . ') ' . $this->mysqli->connect_error);
            } else {
